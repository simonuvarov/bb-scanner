...
class MasscanWorker(BaseWorker):
    ...
    def run(self):
        logger.debug('MasscanWorker has started')
        beanstalk = beanstalkc.Connection(host='localhost', port=11300)
        beanstalk.watch('masscan')
        beanstalk.ignore('default')

        while True:
            job = beanstalk.reserve()

            logger.debug("MasscanWorker has taken a job for {}", job.body)
            job_json = json.loads(job.body)

            host = job_json['host']

            mas = PortScanner()
            result = mas.scan(host)

            db = Database()
            db.update_hosts(result)
            db.close_connection()
...
class PortScanner(object):
    ...
    def scan(self, hosts='127.0.0.1', ports=PORTS, arguments='', sudo=False):
        # Launch scan
        args = [self._masscan_path, '-oJ', '-', hosts, '-p', ports]
        ...
        # wait until finished
        # get output
        masscan_out, masscan_err = p.communicate()
        masscan_out = bytes.decode(masscan_out)
        masscan_err = bytes.decode(masscan_err)
        if masscan_out == "":
            return []
        else:
            return self.parse_json_output(masscan_out)

    def parse_json_output(self, masscan_file_content):
        """
        Parse the output file of masscan
        :param path: path to the outputfile
        :return: HostList object
        """

        result = HostList()
        t = timestamp()
        # Dirty hack because masscan uses bad json format
        masscan_file_content = masscan_file_content.replace('\r\n', '')
        masscan_file_content = masscan_file_content.replace('{  ', '{')
        masscan_file_content = masscan_file_content.replace('},]', '}]')
        entries = json.loads(masscan_file_content)
        for entry in entries:
            host = SingleHost(ip=entry['ip'], timestamp=t)
            ip = entry['ip']

            # It does not supports ipv6 yet
            if not is_valid_ipv4_address(ip):
                entries.remove(entry)

            for port in entry['ports']:
                new_port = SinglePort(port=str(port['port']), state=port['status'], reason=port['reason'],
                                      timestamp=t)
                host.new_port(new_port)
            result.new_host(host)
        return result
